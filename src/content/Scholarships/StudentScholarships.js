...
function StudentScholarships() {
        ...
        if(month < 10 && date > 9){
         ...
         setDate(forday) 
            ...
            axios.get("http://localhost:3001/getSchohalarList").then((response)=>{
                ...
                axios.get('http://localhost:3001/getStudentList').then((res) => {
    ...
    const transitions = useTransition(location.pathname == '/HomeScholarships/StudentScholarships', {
        from: { opacity: 0 },
      ...
      return transitions(
            ...
            <div style={{marginLeft:'10px'}}>
                <InputLabel style={{marginLeft:'10px'}} id="demo-simple-select-outlined-label" >ทุน</InputLabel>
                <Select value={scholarshipName} labelId="demo-simple-select-outlined-label" id="demo-simple-select-outlined" label="Age" variant="outlined" onChange={(e)=>{SelectByScholar(e)}} style={{width:'100px',height:"40px",outline:'none',background:'transparent'}}>
                {dateScholarships.map((data,index)=>(
                    <MenuItem  key={index} value={data.id} name={data.scholarship_name} >{data.scholarship_name}</MenuItem>
                ))}
                </Select>
                ...
                <TableContainer  style={{backgroundColor:'#EAF1F4',borderRadius:'15px'}} >
                    ...
                    <TableBody>
                        ...
                        <StyledTableRow key={index}>                       
                            <StyledTableCell align="left" width="100px">{dataList.student_id}</StyledTableCell>
                            <StyledTableCell align="center" width="100px">{dataList.name} {dataList.lastname}</StyledTableCell>
                            <StyledTableCell align="center" width="100px">{dataList.ad_name} {dataList.ad_lastname}</StyledTableCell>
                ...
                >
                    <DialogTitle id="alert-dialog-title">{"เพิ่มนักสึกษาทุน"}</DialogTitle>
                            ...
                            <div style={{marginTop:'10px'}}>
                                ...
                                {suggestion.map((data, index) => (
                                      <div
                                          ...
                                          key={index}>{data.id}: {data.name} {data.lastname}
                                      </div>
                                  ))}
                            </div>
                        </DialogContentText>
